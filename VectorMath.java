...
public final class VectorMath {
    ...
    // ---- ARRAY OPS FOR PHYSICS BATCHES ----

    /** Sums all Vector2D elements in an array into a single Vector2D (pure). */
    ...
    /** Sums all Vector2F elements in an array into a single Vector2F (pure). */
    ...
    /** Sums all Vector2I elements in an array into a single Vector2I (pure). */
    ...
    /** Multiply each element by scalar (mutating). For Vector2D uses mul(double). */
    ...
    /** Multiply each element by scalar (mutating) for Vector2F (uses scale). */
    ...
    /** Multiply each element by scalar (mutating) for Vector2I (uses scale). */
    ...
    /** Normalize each Vector2D in-place using normalizeInPlace(). Safe-zero preserved. */
    ...
    /** Normalize each Vector2F in-place using normalize(). Safe-zero preserved. */
    ...
    /**
     * Lerp arrays a -> b into out[] (mutating out). Arrays must be non-null and same length.
     * For Vector2D (double precision).
     */
